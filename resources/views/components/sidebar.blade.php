...
<aside x-data="{ collapsed: false, init(){ try{ this.collapsed = localStorage.getItem('sidebar-collapsed') === '1' }catch(e){} }, toggle(){ this.collapsed = !this.collapsed; try{ localStorage.setItem('sidebar-collapsed', this.collapsed ? '1' : '0') }catch(e){} } }"
    x-init="init()"
   ...
   <div class="h-[56px] flex items-center border-b border-white/10 px-3">
    ...
    <div class="flex-1 overflow-y-auto px-4 pb-4 mt-4 space-y-7">
        ...
        <div class="pt-4 border-t border-white/10">
            <div x-show="!collapsed" class="text-[11px] font-semibold tracking-wider mb-3 text-blue-100/80">MANAJEMEN
                TOKO</div>
            ...
            @php $openItem = $open(['item.*','item.kategori.*']); @endphp
            <div x-data="{ open: {{ $openItem ? 'true' : 'false' }} }" class="mt-2">
                ...
                <div x-show="open && !collapsed" x-transition class="mt-1 pl-6 space-y-1">
                    @php $on = $is('item.kategori.*'); @endphp
                    <a href=""
                        class="block px-3 py-2 rounded-md text-[13px] transition {{ $on ? 'bg-white text-[#344579] font-semibold' : 'text-white/80 hover:bg-white/5' }}">
                        <span class="flex items-center gap-3">
                            <span class="inline-block w-[3px] h-5 rounded {{ $on ? 'bg-white' : 'bg-transparent' }}"></span>
                    ...
                    @php $on = $is('item.index') || $is('items.*'); @endphp
                    <a href=""
                        class="block px-3 py-2 rounded-md text-[13px] transition {{ $on ? 'bg-white text-[#344579] font-semibold' : 'text-white/80 hover:bg-white/5' }}">
                        <span class="flex items-center gap-3">
                            <span class="inline-block w-[3px] h-5 rounded {{ $on ? 'bg-white' : 'bg-transparent' }}"></span>
