...
<aside
    x-data="{
        collapsed: false,
        init(){
            try { this.collapsed = localStorage.getItem('sidebar-collapsed') === '1' }
            catch(e){}
        },
        toggle(){
            this.collapsed = !this.collapsed;
            try { localStorage.setItem('sidebar-collapsed', this.collapsed ? '1' : '0') }
            catch(e){}
        }
    }"
    x-init="init()"
    :class="collapsed ? 'w-[64px]' : 'w-[250px]'"
    ...
    <div class="flex-1 overflow-y-auto px-4 pb-4 mt-4 space-y-7">
        ...
        <div>
            ...
            <a href="{{ route('penjualan.index') }}"
               ...
               :class="{ 'justify-center': collapsed }">
                ...
                <span x-show="!collapsed" class="font-medium">Penjualan</span>
            </a>
            ...
            <div x-data="{ open: {{ $openPembelian ? 'true' : 'false' }} }" class="mt-2">
                <button @click="open=!open"
                    ...
                    <i x-show="!collapsed" class="fa-solid fa-chevron-down text-white/60 transition-transform duration-200"
        ...
        <div class="pt-4 border-t border-white/10">
            <div x-show="!collapsed" class="text-[11px] font-semibold tracking-wider mb-3 text-blue-100/80">MANAJEMEN TOKO</div>
                ...
                <div x-show="open && !collapsed" x-transition class="mt-1 pl-6 space-y-1">
                    ...
                    <a href="{{ route('items.categories.index') }}"
                       class="block px-3 py-2 rounded-md text-[13px] transition {{ $onKategori ? 'bg-white text-[#344579] font-semibold' : 'text-white/80 hover:bg-white/5' }}">
                        <span class="flex items-center gap-3">
                            <span class="inline-block w-[3px] h-5 rounded {{ $onKategori ? 'bg-white' : 'bg-transparent' }}"></span>
                    ...
                    <a href="{{ route('items.index') }}"
                       class="block px-3 py-2 rounded-md text-[13px] transition {{ $onDaftar ? 'bg-white text-[#344579] font-semibold' : 'text-white/80 hover:bg-white/5' }}">
                        <span class="flex items-center gap-3">
                            <span class="inline-block w-[3px] h-5 rounded {{ $onDaftar ? 'bg-white' : 'bg-transparent' }}"></span>
        ...
        {{-- MANAJEMEN PENGGUNA --}}
        <div class="pt-4 border-t border-white/10">
            <div x-show="!collapsed" class="text-[11px] font-semibold tracking-wider mb-3 text-blue-100/80">MANAJEMEN PENGGUNA</div>
        ...
        {{-- LAINNYA --}}
        <div class="pt-4 border-t border-white/10">
            <div x-show="!collapsed" class="text-[11px] font-semibold tracking-wider mb-3 text-blue-100/80">LAINNYA</div>
