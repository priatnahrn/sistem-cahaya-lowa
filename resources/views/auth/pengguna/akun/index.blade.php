...
@section('content')
    ...
    <div id="user-page" x-data="userPage()" x-init="init()" class="space-y-6">
        ...
        <div
            ...
            <div class="flex items-center gap-3">
                <div class="relative">
                    ...
                    <input type="text" placeholder="Cari pengguna..." x-model="q"
        ...
        <div x-show="showFilter" x-collapse x-transition class="bg-white border border-slate-200 rounded-xl px-6 py-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" placeholder="Cari username..." x-model="filters.username"
                        class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm text-slate-700
                       focus:outline-none focus:ring-2 focus:ring-[#344579]/20 focus:border-[#344579]">
                </div>
            </div>
            ...
            <div class="flex items-center justify-between mt-4 pt-4 border-t border-slate-200">
                <div class="text-sm text-slate-600">
                    <span x-text="filteredTotal()"></span> dari <span x-text="data.length"></span> pengguna
                </div>
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr class="text-center text-slate-600">
                            ...
                            <th class="px-4 py-3 text-left cursor-pointer" @click="toggleSort('name')">
                                Nama
                                <i class="fa-solid" :class="sortIcon('name')"></i>
                            </th>
                            <th class="px-4 py-3 text-left cursor-pointer" @click="toggleSort('username')">
                                Username
                    ...
                    <tbody>
                            ...
                            <tr class="hover:bg-slate-50 text-slate-700 border-b border-slate-200">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <template x-for="role in r.roles" :key="role">
                                        <span
                                            class="inline-flex items-center gap-2 text-xs font-medium px-2.5 py-1 rounded-full mr-1"
                                ...
                                <td class="px-4 py-3 text-center text-slate-600" x-text="fmtTanggal(r.created_at)"></td>
                        ...
                        <tr x-show="filteredTotal()===0" class="text-center text-slate-500">
                            <td colspan="7" class="px-4 py-8">
                                ...
                                <p class="text-slate-400">Tidak ada data pengguna.</p>
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="p-6 bg-white">
                    <p class="text-slate-700 leading-relaxed">
                        ...
                        <span class="font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded"
                            x-text="deleteItem.username"></span>?
                    </p>
                    ...
                    <div class="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2">
                        ...
                        <div class="text-sm text-amber-700">
                            ...
                            <p class="mt-1">Tindakan ini akan menghapus pengguna secara permanen. Proses ini
    ...
    <script>
        function userPage() {
            return {
                ...
                get availableRoles() {
                    const roles = new Set();
                    this.data.forEach(u => u.roles.forEach(r => roles.add(r)));
                ...
                roleBadgeClass(role) {
                    if (role === 'super-admin') return 'bg-purple-50 text-purple-700 border border-purple-200';
                    if (role === 'admin') return 'bg-blue-50 text-blue-700 border border-blue-200';
                    if (role === 'manager') return 'bg-green-50 text-green-700 border border-green-200';
                ...
                async doDelete() {
                    ...
                    const url = `/users/${this.deleteItem.id}`;
                    ...
                    try {
                        ...
                        if (res.ok && result.success) {
                            ...
                            this.showNotification('success', result.message || 'Pengguna berhasil dihapus');

                            if (this.currentPage > this.totalPages()) {
                                this.currentPage = this.totalPages();
                            }
                        } else {
                            this.showNotification('error', result.message || 'Gagal menghapus pengguna');
                        }
