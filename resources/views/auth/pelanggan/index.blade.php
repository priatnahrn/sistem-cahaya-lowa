...
@section('content')
    ...
    <div x-data="pelangganPage()" x-init="init()" class="space-y-6">
        ...
        <div x-show="showFilter" x-collapse x-transition
            ...
            <div>
                <label class="block text-sm text-slate-500 mb-1">Kontak</label>
                    ...
                    <tbody>
                        <template x-for="(r, idx) in pagedData()" :key="r.id">
                            <tr class="hover:bg-slate-50 text-slate-700 border-b border-slate-200">
                                ...
                                <td class="px-4 py-3">
                                    <a :href="r.url" class="text-[#344579] font-semibold hover:underline" x-text="r.nama_pelanggan" @click="openActionId = null"></a>
                                </td>
    ...
    <script>
        function pelangganPage() {
            return {
                ...
                // sorting state
                sortBy: 'nama_pelanggan', // default kolom
                ...
                init() {
                    ...
                    // prevent issues on page restore
                ...
                doDelete() {
                    ...
                    // NOTE: saat ini hanya client-side. Untuk panggil backend:
                    // fetch(`/pelanggan/${this.deleteItem.id}`, { method: 'DELETE', headers: {'X-CSRF-TOKEN': '...'} })
                    //   .then(() => { /* refresh atau hapus client-side */ })
                },
