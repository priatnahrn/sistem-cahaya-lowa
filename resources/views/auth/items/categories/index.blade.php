...
@section('content')
        ...
        <template x-for="(t, i) in items" :key="t.id">
            <div x-show="t.show" x-transition class="flex items-start gap-3 rounded-md border px-4 py-3 shadow text-sm"
                ...
                <i class="fa-solid text-lg mt-0.5"
                    :class="t.type === 'success' ? 'fa-circle-check' : 'fa-circle-xmark'"></i>
                <div>
    ...
    <div x-data="kategoriPage()" x-init="init()" class="space-y-6">
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr class="text-center text-slate-600">
                            ...
                            {{-- ✅ Kolom aksi hanya muncul jika ada permission delete (karena detail bisa diakses via link nama) --}}
                            @can('kategori_items.delete')
                    ...
                    <tbody>
                        <template x-for="(r, idx) in pagedData()" :key="r.id">
                            <tr class="hover:bg-slate-50 text-slate-700 border-b border-slate-200 text-center">
                                ...
                                {{-- ✅ Dropdown hanya muncul jika user punya permission delete --}}
                                @can('kategori_items.delete')
                        ...
                        <tr x-show="filteredTotal()===0" class="text-center text-slate-500">
                            <td colspan="3" class="px-4 py-8">
                                ...
                                <p class="text-slate-400">Tidak ada data kategori.</p>
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="p-6 bg-white">
                    ...
                    <div class="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2">
                        ...
                        <div class="text-sm text-amber-700">
                            ...
                            <p class="mt-1">Tindakan ini akan menghapus data kategori secara permanen. Proses ini
        ...
        {{-- ✅ Floating Dropdown - Hanya untuk Hapus --}}
        <div x-cloak x-show="dropdownVisible" x-transition.origin.top.right id="floating-dropdown" data-dropdown
            ...
            {{-- ✅ Menu Detail - Semua user bisa akses (tapi di detail page ada conditional untuk form edit) --}}
            ...
            {{-- ✅ Menu Hapus - Hanya yang punya permission delete --}}
            @can('kategori_items.delete')
                ...
                async doDelete() {
                    ...
                    try {
                        ...
                        if (res.ok) {
                            ...
                            const toastEl = document.querySelector('[x-data="toasts()"]');
                            if (toastEl?.__x?.$data) {
                                toastEl.__x.$data.push('success', 'Kategori berhasil dihapus.');
                            }
                        } else {
                            const toastEl = document.querySelector('[x-data="toasts()"]');
                            if (toastEl?.__x?.$data) {
                                toastEl.__x.$data.push('error', 'Gagal menghapus kategori.');
                            }
                        }
                    } catch (e) {
                        ...
                        const toastEl = document.querySelector('[x-data="toasts()"]');
                        if (toastEl?.__x?.$data) {
                            toastEl.__x.$data.push('error', 'Terjadi error server.');
                        }
                    }
