...
@section('title', 'Lihat Detail Item')
...
@section('content')
    <div class="space-y-6 w-full" x-data="itemsEdit()" x-init="init()">
        ...
        <div class="flex flex-wrap items-center gap-2 text-sm">
            <a href="{{ route('items.index') }}" class="text-slate-500 hover:underline">Item</a>
        ...
        <form action="{{ route('items.update', $item->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div x-show="currentStep === 1" x-cloak class="bg-white border border-slate-200 rounded-xl p-6 w-full">
                ...
                {{-- Hidden input untuk index satuan utama (hanya sekali) --}}
                <input type="hidden" name="satuan_primary_index" x-model="primarySatuanIndex">
            ...
            <div class="flex flex-col sm:flex-row justify-end mt-4 gap-3">
                ...
                <button type="submit" :disabled="!canSubmit()"
                    :class="canSubmit() ? 'bg-[#334579] hover:bg-[#2d3e6f] text-white px-4 py-2 rounded-lg w-full sm:w-auto cursor-pointer' :
                        'bg-slate-300 cursor-not-allowed text-white px-4 py-2 rounded-lg w-full sm:w-auto cursor-not-allowed'">
                    Simpan Perubahan
                </button>
            </div>
        </form>
    ...
    <script>
        function itemsEdit() {
            return {
                ...
                // initial data (deep copy supaya bisa dibandingkan)
                initialData: {},
                ...
                canSubmit() {
                    ...
                    return this.isChanged(); // hanya true kalau ada perubahan
                }
            }
        }
    </script>
