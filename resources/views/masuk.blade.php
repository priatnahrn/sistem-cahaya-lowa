...
<head>
    ...
    <style>
        ...
        :root{--primary:#344579;--primary-2:#40548f;--error:#F4522E}
        ...
        .ornament-svg { image-rendering: optimizeQuality; }
    </style>
</head>
...
<body class="min-h-screen bg-gradient-to-b from-[#f3f6fb] to-[#eef3f8] flex items-center justify-center font-sans">
    ...
    <div x-data class="fixed top-6 right-6 space-y-3 z-50 w-80">
        @if (session('success'))
            <div x-data="{ show: true }" x-show="show" x-transition class="rounded-md bg-green-50 border border-green-200 text-green-800 px-4 py-3 shadow">
        ...
        @if (session('error'))
            <div x-data="{ show: true }" x-show="show" x-transition class="flex items-start gap-3 rounded-md border px-4 py-3 shadow text-sm" style="background-color:#FFEAE6; border-color:var(--error); color:var(--error);">
        ...
        @if ($errors->any() && !$errors->has('username') && !$errors->has('password'))
            <div x-data="{ show: true }" x-show="show" x-transition class="rounded-md bg-red-50 border border-red-200 text-red-800 px-4 py-3 shadow">
                <div class="flex items-start gap-3">
    ...
    <main class="w-full max-w-6xl mx-4 md:mx-6 lg:mx-8">
        <div class="bg-white/90 backdrop-blur rounded-2xl shadow-lg border border-slate-100 overflow-hidden md:flex md:items-stretch noise-pattern">

            {{-- Left / Brand panel (dengan ornamen & tekstur terfokus di area brand) --}}
            <aside class="hidden md:block md:w-2/5 lg:w-[42%] p-10 text-white relative paper-emboss" style="background: linear-gradient(135deg,var(--primary),var(--primary-2));">
                <div class="max-w-md relative z-20">
                    <div class="relative inline-block mb-3">
                        <!-- decorative textured badge behind the logo -->
                        <div class="absolute -inset-3 rounded-xl transform rotate-3 pointer-events-none opacity-60" aria-hidden="true">
                            <!-- layered SVG texture: halftone + faint rings -->
                            <svg class="w-full h-full ornament-svg" viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
                                <defs>
                        ...
                        <div class="w-14 h-14 rounded-lg bg-white/6 flex items-center justify-center relative z-10">
                            <svg width="30" height="30" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <path d="M6 12c0-3 2.5-5 6-5s6 2 6 5-2.5 5-6 5-6-2-6-5z" stroke="white" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                        </div>
                    </div>
                    ...
                    <p class="mt-5 text-sm text-white/90 leading-relaxed">Masuk untuk mengelola stok, transaksi, dan laporan. Akses cepat, aman, dan responsif.</p>
                ...
                <div class="absolute inset-0 pointer-events-none z-10">
                    <!-- radial soft glow -->
                    <svg class="absolute -right-10 -top-10 opacity-20" width="260" height="260" viewBox="0 0 260 260" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <defs>
                    ...
                    <svg class="absolute left-6 bottom-6 opacity-8" width="220" height="120" viewBox="0 0 220 120" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <defs>
                            <pattern id="p1" width="20" height="20" patternUnits="userSpaceOnUse" patternTransform="rotate(25)">
                    ...
                    <svg class="absolute -left-10 -bottom-8 opacity-30" width="140" height="140" viewBox="0 0 140 140" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="30" cy="30" r="28" fill="white" opacity="0.02" />
                        <circle cx="110" cy="90" r="36" fill="white" opacity="0.03" />
                    </svg>

                    <!-- subtle grid dots -->
                    <svg class="absolute right-6 bottom-20 opacity-10" width="120" height="120" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <defs>
                ...
                <div class="absolute inset-0 bg-gradient-to-b from-transparent to-black/2 opacity-5 pointer-events-none z-0"></div>
            </aside>
            ...
            <section class="w-full md:w-3/5 lg:w-[58%] p-8 md:p-12 flex items-center justify-center">
                <div class="w-full max-w-md">
                    ...
                    <p class="text-sm text-slate-600 mt-2">Masukkan <strong>username</strong> dan <strong>password</strong> Anda untuk melanjutkan.</p>
                    ...
                    <form method="POST" action="{{ route('login') }}" class="space-y-5 mt-6" novalidate>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500"><i class="fa-solid fa-user"></i></span>
                                <input id="username" name="username" type="text" placeholder="Username" value="{{ old('username') }}" class="w-full pl-12 pr-4 py-3 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#344579]/20 focus:border-[#344579]" aria-invalid="{{ $errors->has('username') ? 'true' : 'false' }}" aria-describedby="username-error" />
                            </div>

                            @error('username')
                                <div id="username-error" class="mt-2 text-sm text-[var(--error)]">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div x-data="{ show: false }">
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500"><i class="fa-solid fa-key"></i></span>
                                <input :type="show ? 'text' : 'password'" id="password" name="password" placeholder="Password" class="w-full pl-12 pr-12 py-3 rounded-lg bg-slate-50 border border-slate-200 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#344579]/20 focus:border-[#344579]" aria-invalid="{{ $errors->has('password') ? 'true' : 'false' }}" aria-describedby="password-error" />

                                <button type="button" class="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-md text-slate-500" @click="show = !show" :aria-label="show ? 'Sembunyikan password' : 'Tampilkan password'">
                            ...
                            @error('password')
                                <div id="password-error" class="mt-2 text-sm text-[var(--error)]">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        {{-- Submit --}}
                        <div>
                            <button type="submit" class="w-full py-3 rounded-lg font-semibold text-white shadow-md" style="background: linear-gradient(90deg,var(--primary),var(--primary-2));">Masuk</button>
                        </div>

                        {{-- Help --}}
                        <div class="text-center mt-2">
                            <div class="text-sm text-slate-500">Butuh bantuan? <a href="#" class="font-semibold text-[#344579]">Hubungi admin</a></div>
                        </div>

                        <div class="text-center mt-4 text-sm text-slate-400">© {{ date('Y') }} CV Cahaya Lowa — Semua hak dilindungi.</div>
                    </form>
                </div>
            </section>
        </div>
